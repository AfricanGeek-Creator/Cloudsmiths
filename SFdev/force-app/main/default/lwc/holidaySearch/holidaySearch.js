...
import insertSAIDRecord from '@salesforce/apex/SAIDSearchHandler.insertSAIDRecord';
export default class HolidaySearch extends LightningElement {
    ...
    decodeIdNumber(idNumber) {
        ...
        if (calculatedChecksum !== checksum) {
            ...
            const Data=JSON.stringify(SAID__C);
          
           insertSAIDRecord({Data}).then(() => {
            console.log('Record inserted successfully:', result);
        })
        .catch(error => {
            console.log('Record fail:', error);
            alert("FAIL!!!!")
        });
     // Invalid checksum
        }
